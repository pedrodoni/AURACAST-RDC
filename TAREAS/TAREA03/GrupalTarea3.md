...
Fórmula de pérdida en el espacio libre ($L_{dB}$) para una antena isotrópica:

$L_{dB} = 20 \log(f) + 20 \log(d) - 147,56$

Sabiendo que $\log(A \cdot B) = \log(A) + \log(B)$ y que $\log(2) \approx 0,30103$, podemos demostrarlo de la siguiente manera:

<u>1. Duplicando la frecuencia ($2f$)</u>

Si dejamos la distancia constante y la nueva frecuencia es $2f$, la ecuación nos queda:

$L_{dB}' = 20 \log(2f) + 20 \log(d) - 147,56$

Separando el término del logaritmo:

$L_{dB}' = 20 (\log(2) + \log(f)) + 20 \log(d) - 147,56$

$L_{dB}' = 20 \log(2) + [20 \log(f) + 20 \log(d) - 147,56]$

Lo que nos queda entre corchetes es exactamente la fórmula de la pérdida original ($L_{dB}$). Reemplazando eso y el valor de $\log(2)$:

$L_{dB}' = 20(0,30103) + L_{dB}$

$L_{dB}' \approx 6,02 \text{ dB} + L_{dB}$

Por lo tanto, la atenuación aumenta unos 6 dB respecto a la original.

<u>2. Duplicando la distancia ($2d$)</u>

Manteniendo $f$ constante y usando $2d$:

$L_{dB}'' = 20 \log(f) + 20 \log(2d) - 147,56$

$L_{dB}'' = 20 \log(f) + 20 (\log(2) + \log(d)) - 147,56$
...
Entonces, al duplicar cualquiera de las dos variables, se le suman $\approx 6 \text{ dB}$ a la pérdida, lo que equivale a la atenuación de la potencia recibida.
...
- Frecuencia ($f$) = $30 Hz$
- Velocidad de la luz ($c$) = $3 \times 10^8 \frac{ m}{s}$
- Fórmula de la antena = $\frac{\lambda}{2}$

<u>Cálculo de la longitud de onda ($\lambda$):</u>

$\lambda = \frac{c}{f} = \frac{3 \cdot 10^8 \frac{m}{s}}{30 Hz} = 10.000.000 m (10.000 km)$

<u>Cálculo del tamaño de la antena:</u>

Longitud de antena:

$ \frac{\lambda}{2} = \frac{10.000.000m}{2} = 5.000.000 m(5.000 km)$

**Respuesta:**

La longitud típica de la antena debe ser de 5.000 kilómetros.
...
$f = 300 Hz, c = 3 \cdot 10^{8} \frac{m}{s}$

<u>Cálculo de $\lambda$:</u>
$\lambda = \frac{c}{f} = \frac{3 \cdot 10^8}{300} = 1.000.000m$

<u>Longitud de la antena ($\lambda / 2$):</u>
Longitud $= \frac{1.000.000}{2} = 500.000 m$

La longitud de la antena para enviar una señal de 300 Hz debe ser de 500.000 metros.

b)

<u>Datos: </u>

Longitud de la antena $= 1 m$, por lo que $\frac{\lambda}{2} = 1 \implies \lambda = 2 m$.

<u>Cálculo de la frecuencia ($f$):</u>
$f = \frac{c}{\lambda} = \frac{3 \cdot 10^8}{2} = 150.000.000 Hz (150 MHz)$

Para poder utilizar una antena de 1 metro de longitud, se debe utilizar una frecuencia de portadora de 150.000.000 Hz.
...
- Longitud del empaste (antena) = $2,5 mm = 0,0025 m$
- Condición de la antena = La longitud es igual a la mitad de la longitud de onda ($\frac{\lambda}{2}$)
- Velocidad de la luz ($c$) = $3 \cdot 10^8 \frac{m}{s}$

<u>Cálculo de la longitud de onda ($\lambda$):</u>

$\frac{\lambda}{2} = 0,0025 m \implies \lambda = 0,0025 \cdot 2 = 0,005 m$

<u>Cálculo de la frecuencia ($f$):</u>

$f = \frac{c}{\lambda} = \frac{3 \cdot 10^8 \text{ m/s}}{0,005 m} = 60.000.000.000 \text{ Hz} = 60 \text{GHz}$

La persona recibiría una frecuencia de 60 GHz.
